    ...
    return (
        <div className="cart">
                    ...
                    cartItems.map(item=>
                        <li>
                            ...
                            <div>Qty:<select value={item.qty} onChange={(e)=>dispatch(addToCart(item.product,e.target.value))}>
                                ...
                                <button type="button" className="button" onClick={()=>removeFromCartHandler(item.product)}>Remove Item</button>
                                </div>
            ...
            <div className="cart-action">
                ...
                <button className="button" onClick={checkoutHandler} disabled={cartItems.length===0}>Proceed to CheckOut</button>
            </div>
        </div>
    )
}
